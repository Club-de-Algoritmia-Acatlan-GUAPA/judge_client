...
import LayoutContest, {
  useContest,
  contestGetLayout,
} from '@components/layouts/LayoutContest'
...
const Problems: NextPageWithLayout = () => {
  const multifetcher = (url: any) => fetch(url).then((r) => r.json())
    ...
    <>
      <div className='flex'>
        <table className='table'>
          ...
          <tbody>
            {dataContest?.problemsInfo.map(
              ({ id, name, author, slug }: ProblemInfo, idx: number) => {
                return (
                  <>
                    <tr>
                      <td>
                      ...
                      <td>
                        <Ty
                          clickable
                          href={`contest/${contest_id}/submit/${slug}`}
                          color='blue'
                          type='h3'
                        >
                          {' '}
                    ...
                    <>
                      <tr>
                        <td>
                          {' '}
                          {elem.status === 'AC' ? (
                            <Ty color='green' type='h3' weight='bold'>
                              {' '}
                              Accepted{' '}
                            </Ty>
                          ) : elem.status === 'WA' || elem.status == 'RTE' ? (
                            <Ty color='red' type='h3' weight='bold'>
                              {' '}
                              Wrong Answer{' '}
                            </Ty>
                          ) : (
                            <Ty color='blue' type='h3'>
                              {' '}
                              ?{' '}
                            </Ty>
                          )}
