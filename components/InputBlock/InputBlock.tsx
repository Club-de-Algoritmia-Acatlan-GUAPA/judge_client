...
const InputBlock: React.FC<{
    ...
    id: string,
    type?: string,
    onValidation?: <T> (a: any) =>
        | { success: true; data: T; }
        | { success: false; error: z.ZodError }
    onChange?: (e: any) => any,
}> = ({
    label, placeholder, text, id,type = "text", onValidation, onChange
}) => {
        ...
        return <>
            <div className={style.container}>
                <label className={style.label} htmlFor={id}> <Ty type="h3" color="primary" >{label}</Ty></label>
                <input
                    ...
                    />
                {
                    text && <label className={style.text}> <Ty color="primary" type="text" sz="sm">{text}</Ty> </label>
                }
            </div>
        </>
    }
